...
class ExtensionManager {
  ...
  async _extractArchive(archivePath, targetDir) {
    ...
    for (const entry of zipEntries) {
      if (!this.validateExtractPath(entry.entryName, targetDir)) {
   ...
   * @returns {Promise<object>} - { success: boolean, warnings: string[] }
   */
  async _installDependencies(extensionDir) {
    ...
    try {
      ...
      return { success: true, warnings: [] }; // No package.json, skip
    }

    // Run npm install with security flags (Requirement 3.8)
    return new Promise((resolve, reject) => {
      const npm = spawn('npm', [
        'install',
        '--production',
   ...
   */
  async _runNpmAudit(extensionDir) {
    return new Promise((resolve) => {
      ...
      npm.on('close', () => {
        try {
          ...
          resolve({
            hasVulnerabilities: highVulns > 0 || criticalVulns > 0,
            high: highVulns,
            critical: criticalVulns,
            report: auditResult
          });
        } catch (error) {
          // If audit fails, continue without blocking
          resolve({ hasVulnerabilities: false, high: 0, critical: 0 });
        }
      });
  ...
  /**
   * Install extension (Requirement 3)
   * @param {object} options - Installation options
   * @param {string} options.id - Extension ID (owner/repo)
   * @param {string} options.downloadUrl - Release archive URL
   * @param {string} options.version - Version tag
   * @param {string} options.checksum - Optional SHA-256 checksum
   * @param {string} options.currentVersion - Current application version
   * @param {object} options.manifest - Extension manifest
   * @returns {Promise<object>} - Installation result
   */
  async installExtension(options) {
    const { id, downloadUrl, version, checksum, currentVersion, manifest } = options;
    ...
    // Check version compatibility (Requirement 3.1)
    if (manifest.engines && manifest.engines['claude-code']) {
      const compatible = this.checkVersionCompatibility(
        manifest.engines['claude-code'],
        currentVersion
      );

      if (!compatible) {
        throw new Error(
          `Incompatible version. Requires ${manifest.engines['claude-code']}, current is ${currentVersion}`
        );
      }
    }

    // Check if already installed (Requirement 3.2)
    const existing = this.checkExistingExtension(id);
    if (existing) {
      throw new Error(existing.message);
    }

    const tempDir = path.join(os.tmpdir(), `ext-install-${Date.now()}`);
    const archivePath = path.join(tempDir, 'archive.zip');
    const extractDir = path.join(this.extensionsDir, id);

    try {
      // Create temp directory
      await fsPromises.mkdir(tempDir, { recursive: true, mode: 0o700 });

      // Download archive (Requirement 3.4)
      await this._downloadFile(downloadUrl, archivePath);

      // Verify checksum if provided (Requirement 3.5)
      if (checksum) {
        const valid = await this._verifyChecksum(archivePath, checksum);
        if (!valid) {
      ...
      await fsPromises.mkdir(extractDir, { recursive: true, mode: 0o700 });

      // Extract archive with Zip Slip protection (Requirement 3.7, 3.8)
      await this._extractArchive(archivePath, extractDir);

      // Install npm dependencies (Requirement 3.9)
      const installResult = await this._installDependencies(extractDir);

      // Run npm audit (Requirement 3.10)
      const auditResult = await this._runNpmAudit(extractDir);

      // Update registry (Requirement 3.11)
      const extension = new Extension({
        name: manifest.name,
        owner: { login: id.split('/')[0] },
        description: manifest.description,
        html_url: manifest.repository,
        topics: manifest.keywords || []
      }, {
        id,
        type: manifest.type,
        version,
      ...
      return {
        success: true,
        extension,
        warnings: auditResult.hasVulnerabilities
          ? [`Security vulnerabilities detected: ${auditResult.high} high, ${auditResult.critical} critical`]
          : []
      };

    } catch (error) {
      // Cleanup on failure (Requirement 3.13)
      try {
        await fsPromises.rm(tempDir, { recursive: true, force: true });
  ...
  /**
   * Uninstall extension (Requirement 4)
   * @param {string} id - Extension ID
   * @returns {Promise<boolean>}
   */
  async uninstallExtension(id) {
    ...
    const extensionDir = path.join(this.extensionsDir, id);

    try {
      // Remove extension files (Requirement 4.1)
      await fsPromises.rm(extensionDir, { recursive: true, force: true });

      // Update registry (Requirement 4.3)
