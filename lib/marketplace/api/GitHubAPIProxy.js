...
class GitHubAPIProxy {
  ...
  /**
   * Search GitHub repositories by query or topic
   * @param {string} queryOrTopic - Search query or topic to search for
   * @param {object} options - Search options
   * @returns {Promise<Array>} - Array of repository objects
   */
  async searchRepositories(queryOrTopic, options = {}) {
    // Check if this is already a formatted query (contains operators like ':')
    // or if we should treat it as a topic
    let query;
    if (options.useTopic !== false && !queryOrTopic.includes(':') && !queryOrTopic.includes(' ')) {
      // Use topic search for single-word queries
      query = `topic:${queryOrTopic}`;
    } else {
      // Use regular search for complex queries or when explicitly requested
      query = queryOrTopic;
    }
