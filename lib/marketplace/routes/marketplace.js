  ...
  async listExtensions(req, res) {
    try {
      ...
      for (const topic of topics) {
        ...
        allExtensions.push(...results);
      }

      // Mark installed extensions
      allExtensions.forEach(ext => {
        const installedExt = installed.find(i => i.id === `${ext.owner.login}/${ext.name}`);
        if (installedExt) {
  ...
  async getExtension(req, res, id) {
    try {
      ...
      if (installedExt && installedExt.isInstalled) {
        repoData.isInstalled = true;
        repoData.installedVersion = installedExt.installedVersion;
      }

      // Get latest release
      try {
        const release = await this.githubProxy.getLatestRelease(owner, repo);
        repoData.latestRelease = release;
      } catch (error) {
        repoData.latestRelease = null;
      }
      ...
      this.sendJSON(res, 200, repoData);
