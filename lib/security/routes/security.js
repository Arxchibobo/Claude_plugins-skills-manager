...
class SecurityRoutes {
  ...
  /**
   * Wrap native Node.js response object to provide Express-style API
   * This allows controllers to use res.status().json() pattern
   */
  wrapResponse(res) {
    ...
    wrapper._statusCode = 200;

    // Express-style status() method
    wrapper.status = function(code) {
      this._statusCode = code;
      return this;
    };

    // Express-style json() method
    wrapper.json = function(data) {
      this.writeHead(this._statusCode, { 'Content-Type': 'application/json' });
      this.end(JSON.stringify(data));
      return this;
    };

    // Express-style send() method
    wrapper.send = function(data) {
      if (typeof data === 'object') {
        return this.json(data);
      }
      this.writeHead(this._statusCode, { 'Content-Type': 'text/plain' });
  ...
  async handleRoute(req, res, url, method) {
    try {
      ...
      // Wrap response object to provide Express-style API for controllers
