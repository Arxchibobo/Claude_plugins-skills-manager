...
class ClaudeIntegration {
  ...
  async runSecurityScan(config) {
    ...
    // 构建命令参数
    // 注意：prompt 必须作为位置参数放在最后，不是 -p 的值
    // 正确用法：claude -p "prompt text" --options
    const args = [
      '-p',                                      // print 模式（非交互式）
      '--output-format', 'json',                 // JSON 输出格式
      '--permission-mode', 'bypassPermissions',  // 绕过权限对话框（加快执行速度）
      '--add-dir', absolutePath,                 // 允许访问扫描目录
      prompt                                     // prompt 作为位置参数
    ];

    // Execute command
    try {
      // 记录调试信息
      console.log(`[ClaudeIntegration] 执行安全扫描命令: ${this.cliCommand} -p --output-format json --add-dir ${absolutePath} "[prompt]"`);
      console.log(`[ClaudeIntegration] 工作目录: ${process.cwd()}`);
      console.log(`[ClaudeIntegration] 超时设置: ${this.config.timeout}ms (${Math.round(this.config.timeout / 1000)}秒)`);
      console.log(`[ClaudeIntegration] Prompt 长度: ${prompt.length} 字符`);
      ...
      // 不缓存失败的扫描结果
      console.error(`[ClaudeIntegration] 安全扫描失败:`, {
        code: error.code,
        killed: error.killed,
        signal: error.signal,
        message: error.message,
        stderr: error.stderr ? error.stderr.toString().substring(0, 500) : null,
        stdout: error.stdout ? error.stdout.toString().substring(0, 200) : null
      });
  ...
  async runCodeReview(config) {
    ...
    // 构建命令参数（prompt 作为位置参数放在最后）
    const args = [
      '-p',                                      // print 模式
      '--output-format', 'json',                 // JSON 输出格式
      '--permission-mode', 'bypassPermissions'   // 绕过权限对话框
    ];
