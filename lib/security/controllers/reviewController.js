    ...
    const reviewPromise = claudeIntegration.runCodeReview(reviewConfig)
      .then(cliResult => {
        ...
        if (!cliResult.success) {
          throw new Error(cliResult.error || 'Code review failed');
        }

        // Parse result from CLI output string
        const reviewResult = resultParser.parseReviewResult(cliResult.output, { format: reviewConfig.format });
...
module.exports = {
  ...
  _getActiveReviews: () => activeReviews // For testing
};
