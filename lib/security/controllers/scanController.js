...
let claudeIntegration = new ClaudeIntegration();
let resultParser = new ResultParser();
...
async function startScan(req, res) {
  try {
    ...
    // Generate scan ID first (before promise)
    ...
    const scanPromise = claudeIntegration.runSecurityScan(scanConfig)
      ...
      .catch(error => {
        // Mark scan as failed (use existing scanId)
        ...
        // Don't re-throw error to prevent server crash
        // The error is already stored in activeScans
      });
...
module.exports = {
  ...
  _setDependencies: (deps) => {
    if (deps.claudeIntegration) claudeIntegration = deps.claudeIntegration;
    if (deps.resultParser) resultParser = deps.resultParser;
    if (deps.historyManager) historyManager = deps.historyManager;
  },
  _resetDependencies: () => {
  ...
  _getActiveScans: () => activeScans // For testing
};
